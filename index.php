...
if (isset($_REQUEST['questionCount']) && isset($_REQUEST['timeLimit']) && isset($_REQUEST['numberA']) && isset($_REQUEST['numberB']) && isset($_REQUEST['smallestnumber']) && isset($_REQUEST['bigestnumber'])) {
    ...
    if ($smallestNumber > $biggestNumber || $smallestNumber >= 0) {
        ...
        header("Location: index.php?questionCount=10&timeLimit=3&numberA=0&numberB=10&smallestnumber=0&bigestnumber=10");
        exit();
    }
    $numberList = generateRandomArray($questionCount, $numberA, $numberB, $smallestNumber, $biggestNumber);
} else {
    header("Location: index.php?questionCount=10&timeLimit=3&numberA=-10&numberB=10&smallestnumber=0&bigestnumber=10");
    exit();
}
...
<body>
    ...
    <div class="controls">
        <input type="number" id="answerInput" placeholder="Nhập đáp án">
        <button id="checkButton">Kiểm tra</button>
        <button id="pausePlayButton">Bắt đầu</button>
        <button id="nextButton">Kết tiếp</button>
    </div>
